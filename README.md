...
Strong performance under full-data LOSO conditions
Gradual degradation as training data per class is reduced
Significant performance drop at very low data regimes (e.g., cap ≤ 2)
High sensitivity to signer variation in low-data settings

These results highlight the importance of training data diversity for signer-independent recognition.
...
Run experiments by navigating to the respective directory and executing:

python runner.py

Configuration parameters such as dataset path and output directories should be set inside the runner scripts.

11. Data Format
Input data: .h5 pose files
Each file contains frame-wise keypoint data
Labels are inferred from file naming conventions
12. Summary
This system implements a pose-based Transformer architecture for sign language recognition and evaluates its performance under varying data availability and evaluation protocols.

The results emphasize the role of temporal modeling and data diversity in achieving robust signer-independent performance.
```text
